...
def test_save_and_search_with_dummy_db(dummy_db, monkeypatch):
    ...
    e1 = [0.1, 0.2, 0.3]
    e2 = [0.9, 0.1, 0.5]
    s1 = save_embedding_to_db(db=dummy_db, content="c1", embedding=e1, user_id=None, tags=["t"])
    s2 = save_embedding_to_db(db=dummy_db, content="c2", embedding=e2, user_id=None, tags=None)
    out = semantic_search(dummy_db, e1, user_id=None, limit=5)
...
def test_semantic_search_returns_scores_and_order(dummy_db, monkeypatch):
    ...
    q = [1.0, 0.0, 0.0]
    _s_a = save_embedding_to_db(db=dummy_db, content="A", embedding=[1.0, 0.0, 0.0])
    _s_b = save_embedding_to_db(db=dummy_db, content="B", embedding=[0.8, 0.6, 0.0])
    _s_c = save_embedding_to_db(db=dummy_db, content="C", embedding=[0.0, 1.0, 0.0])
    res = semantic_search(dummy_db, q, user_id=None, limit=3)
